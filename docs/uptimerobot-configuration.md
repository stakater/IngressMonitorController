...
UptimeRobot's API v3 is used (`apiURL: https://api.uptimerobot.com/v3`). Authentication is handled automatically by the controller via the `Authorization: Bearer <apiKey>` header.
...
```bash
curl -H "Authorization: Bearer your_api_key" https://api.uptimerobot.com/v3/alert-contacts
```
...
```bash
curl -H "Authorization: Bearer your_api_key" https://api.uptimerobot.com/v3/psps
```
...
```json
{
  ...
  "data": [
    {
      "id": 12345,
      "friendlyName": "my-public-status-page",
      "monitorIds": [],
      "urlKey": "12345678"
    }
  ]
...
```bash
curl -H "Authorization: Bearer your_api_key" https://api.uptimerobot.com/v3/maintenance-windows
```
...
```json
{
    ...
    [
        {
            "id":12345,
            "friendlyName":"Week night maintenance slots",
            "startTime":"22:00",
            "duration":60,
            "value":"1,2,3,4,5"
        }
